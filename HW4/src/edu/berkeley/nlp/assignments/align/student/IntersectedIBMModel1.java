...
public class IntersectedIBMModel1 implements WordAligner {
    ...
    private HashMap<String, Double> tsIndexer = new HashMap<>(10000);
    private HashMap<String, Double> stIndexer = new HashMap<>(10000);

    IntersectedIBMModel1(Iterable<SentencePair> trainingData) {
        int numPairs = 0;
    ...
    public Alignment alignSentencePair(SentencePair sentencePair) {
        ...
        List<String> englishWords = sentencePair.getEnglishWords();
        List<String> frenchWords = sentencePair.getFrenchWords();
        int[] alignmentFrenchGivenEnglish = new int[frenchWords.size()];
        int[] alignmentEnglishGivenFrench = new int[englishWords.size()];
//        double epsilon = 0.1;
//        double nullPrior = epsilon/englishWords.size();
//        double notNullPrior = (1-epsilon)/englishWords.size();

        for (int i = 0; i < frenchWords.size(); i++) {
            double alignProb = tsIndexer.get(frenchWords.get(i).toLowerCase() + "===" + "null");
//            double alignProb = tsIndexer.get(frenchWords.get(i) + "===" + englishWords.get(0)) * nullPrior;
            alignmentFrenchGivenEnglish[i] = -1;
            for (int j = 0; j < englishWords.size(); j++) {
                double currentProb = tsIndexer.get(frenchWords.get(i).toLowerCase() + "===" + englishWords.get(j).toLowerCase());
//                double currentProb = tsIndexer.get(frenchWords.get(i) + "===" + englishWords.get(j)) * notNullPrior;
                if (currentProb > alignProb) {
                    alignProb = currentProb;
                    alignmentFrenchGivenEnglish[i] = j;
                }
            }
//            if(alignIndex != 0) {
//                alignment.addAlignment(alignIndex-1, i, true);
//            }
        }
        for (int i = 0; i < englishWords.size(); i++) {
            double alignProb = stIndexer.get(englishWords.get(i).toLowerCase() + "===" + "null");
            alignmentEnglishGivenFrench[i] = -1;
            for (int j = 0; j < frenchWords.size(); j++) {
                double currentProb = stIndexer.get(englishWords.get(i).toLowerCase() + "===" + frenchWords.get(j).toLowerCase());
                if (currentProb > alignProb) {
                    alignProb = currentProb;
                    alignmentEnglishGivenFrench[i] = j;
                }
            }
        }

        for(int i=0;i<alignmentEnglishGivenFrench.length;i++){
            if(alignmentEnglishGivenFrench[i]!=-1) {
                int frenchPos = alignmentEnglishGivenFrench[i];
                if(alignmentFrenchGivenEnglish[frenchPos]==i)
                    alignment.addAlignment(i, alignmentEnglishGivenFrench[i], true);
            }
        }
        return alignment;
    }
